...
namespace IGREJA_SERES
{
    public partial class FormCadUsuario : Form
    {
        ...
        private void Editar(usuario usuario)
        {
            ...
            usuario.Idperfil = Convert.ToInt32(comboBoxPerfil.SelectedValue);
            if (comboBoxPerfil.SelectedIndex == 0) 
            {
                ...
                {
                    MessageBox.Show("Senhas não compativeis", "Validação", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
                else { 
                try
            {
                usuarioBLL.Editar(usuario);
                MessageBox.Show("Usuário alterado com Suceeso" , "Validação", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        ...
        private void salvar(usuario usuario)
        {
            ...
            if(comboBoxPerfil.SelectedIndex == 0)
            {
                MessageBox.Show("Por favor selecione um perfil" ,"Validação", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            try
            {
                ...
                MessageBox.Show("Usuario Cadastrado com Sucesso", "Validação", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception erro)
            {
        ...
        public void Excluir(usuario usuario)
        {
            ...
            MessageBox.Show(" usuario excluido com sucesso" ,"Validação", MessageBoxButtons.OK, MessageBoxIcon.Information);
            listar();
            limparCampos();
            comboBoxPerfil.SelectedIndex = 0;
        }
        ...
        private void dgvUsuario_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            comboBoxPerfil.SelectedValue = dgvUsuario.CurrentRow.Cells[4].Value.ToString();
